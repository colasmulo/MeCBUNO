...
def Calculer_duo():
    ...
    Planeur_input = Immat_deroul.get()
    pl_index = Planeur_list_str.index(Planeur_input)
    Masse = Planeur_list[pl_index][0] + P1
    Moment = Planeur_list[pl_index][1]*P1 + Planeur_list[pl_index][3]*Planeur_list[pl_index][0]
    CG = round((Moment/Masse)*1000, 2)
    tk.Label(frame, text="Planeur : %s\nPosition du centre de gravité (en mm): %s" % (Planeur_input, CG)).grid(row=5)
...
#Choix type planeur
tk.Label(frame, text="Selectionnez le type de planeur :").grid(row=1)
Type_deroul = ttk.Combobox(frame, values=Planeur_type)
Type_deroul.grid(row=1, column=1)
Type_sel = Type_deroul.get()
#club_mod = __import__(planeur_ddb)
#from Club_sel import Planeur_list, Planeur_list_str
#Immat planeur
tk.Label(frame, text="Selectionnez l'immat concours du planeur :").grid(row=2)
Immat_deroul = ttk.Combobox(frame, values=Planeur_list_str)
Immat_deroul.grid(row=2, column=1)
#Poids pil avant
tk.Label(frame, text="Masse du pilote avant en kilos :").grid(row=3)
Pil_av = tk.Entry(frame)
Pil_av.grid(row=3, column=1)
#Poids pil arriere
tk.Label(frame, text="Masse du pilote arrière en kilos (0 si  pas de pilote arrière):").grid(row=4)
Pil_ar = tk.Entry(frame)
Pil_ar.grid(row=4, column=1)

tk.Button(frame,
            text='Quitter',
            command=frame.quit).grid(row=5,
                                    column=0,
                                    sticky=tk.W,
                                    pady=4)
if Type_sel == "Monoplace":
    tk.Button(frame,
            text='Calculer',
            command=Calculer_mono).grid(row=5,
                                    column=1,
                                    sticky=tk.W,
                                    pady=4)
    ...
    tk.Button(frame,
            text='Calculer',
            command=Calculer_duo).grid(row=5,
                                    column=1,
                                    sticky=tk.W,
                                    pady=4)
